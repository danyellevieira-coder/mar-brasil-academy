...
interface Playlist {
  ...
  isPublished: boolean
  videos: { video: Video; order: number }[]
...
export default function EditPlaylistPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    try {
      ...
      setFormData({
        ...
        videoIds: playlist.videos
          ...
          .map(pv => pv.video.id),
        departmentIds: playlist.access.map(a => a.department.id)
      })
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json()
        setError(data.error || 'Erro ao salvar playlist')
        return
      }

      setSuccess('Playlist atualizada com sucesso!')
      setTimeout(() => router.push('/admin/playlists'), 1500)
    } catch {
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-white mb-2">Editar Playlist</h1>
        <p className="text-[var(--foreground-muted)]">Atualize as informações da playlist</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-between p-4 bg-[var(--surface)] rounded-lg border border-[var(--border)]">
          <div>
            <h3 className="text-white font-medium">Status de Publicação</h3>
